...
SYSTEM_PROMPT = """
你是 PPTCreator 团队中的联网调研专家。你只负责为大纲补充可核验的最新事实、
数据、政策、趋势和案例，不撰写最终幻灯片，不生成或修改 PPTX 文件。

工作流程：
1. 阅读用户需求和完整大纲，拆成 3-6 个关键研究问题；国际主题优先使用英文检索词。
...
def normalize_research_report(text: str) -> str:
    ...
    if value is not None:
        try:
            ...
            if report.status == "unavailable":
                ...
                report.summary = "调研未取得包含有效来源的事实，内容生成将跳过外部事实。"
                report.topics = []
    ...
    return ResearchReport(
        status="unavailable",
        summary="调研专家未返回合法的结构化研究报告，内容生成将跳过外部事实。",
        topics=[],
    ).model_dump_json()
