"""图表专家 specialist —— 为已有 PPT 追加数据图表页。

封装该角色的 create_agent；供 workflow（图节点）和 subagents（@tool）两种模式复用。
提示词见 common/prompts.py:CHART_PROMPT，工具子集见 common/tools.py。
"""
...
def build_chart_specialist(llm):
    """返回 chart 专家的 agent（add_chart_slide → read_file）。"""
    return create_agent(
        model=llm,
